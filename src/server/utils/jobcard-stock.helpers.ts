import { JobCard, PaperStock } from '@/server/models';
...
type StockDoc = InstanceType<typeof PaperStock>;

export const getCoverUsage = (job: JobLike) => {
  if (
    !job ||
...
export const getInnerUsage = (job: JobLike) => {
  if (
    !job ||
    job.paperSource !== 'Company paper' ||
    !job.innerPaper ||
...
export const findCoverStock = async (
  paperStockModel: typeof PaperStock,
  paper: string,
) => {
...
export const syncStockFromJobChange = async (
  ...
  if (oldCover) {
    await applyCoverDelta(
      ...
      {
        partyName:
          (previousJob?.partyName as string) ||
          (previousJob?.companyName as string) ||
          '',
        jobNumber: (previousJob?.jobNumber as string) || '',
