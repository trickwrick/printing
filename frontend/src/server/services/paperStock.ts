import { PaperStock, PaperStockTransaction } from '@/server/models';
import { backfillPaperStockTransactionsIfEmpty } from '@/server/utils/backfillPaperStockTransactions';
...
export async function getTransactions() {
  ...
  return PaperStockTransaction.find().sort({ createdAt: -1 });
}

export async function findAll() {
  return PaperStock.find().sort({ name: 1, gsm: 1 });
}
...
export async function create(body: Record<string, unknown>) {
  ...
  const existing = await PaperStock.findOne({
    name: resolvedName,
  ...
  const newItem = await PaperStock.create({
    name: resolvedName,
...
export async function update(id: string, body: Record<string, unknown>) {
  ...
  const existing = await PaperStock.findById(id);
  if (!existing) {
...
export async function remove(id: string) {
  const deleted = await PaperStock.findByIdAndDelete(id);
  if (!deleted) {
    throw new Error('Item not found');
  }
